...
import { PurchasePreferencesMockRepository } from './purchase-preferences.repository';

@Injectable({ providedIn: 'root' })
export class PurchasePreferencesHttpRepository implements PurchasePreferencesRepository {
  private readonly http = inject(HttpClient);
  private readonly fallback = inject(PurchasePreferencesMockRepository);
  ...
  getInitialState(): Observable<PurchasePreferencesPageState> {
    return this.http
      ...
      .pipe(
        ...
        catchError((error: unknown) => this.fallbackOnNetworkError(error, () => this.fallback.getInitialState()))
      );
  }
  ...
  savePreference(state: PurchasePreferencesPageState): Observable<PurchasePreferencesPageState> {
    return this.http
      ...
      .pipe(
        map(unwrapApiResponse),
        catchError((error: unknown) => this.fallbackOnNetworkError(error, () => this.fallback.savePreference(state)))
      );
  }
  ...
  activateAlert(state: PurchasePreferencesPageState): Observable<PurchasePreferencesPageState> {
    return this.http
      ...
      .pipe(
        map(unwrapApiResponse),
        catchError((error: unknown) => this.fallbackOnNetworkError(error, () => this.fallback.activateAlert(state)))
      );
  }
  ...
  buildSummary(state: PurchasePreferencesPageState): Observable<SummaryPreviewData> {
    return this.http
      ...
      .pipe(
        map(unwrapApiResponse),
        catchError((error: unknown) => this.fallbackOnNetworkError(error, () => this.fallback.buildSummary(state)))
      );
  }

  private fallbackOnNetworkError<T>(error: unknown, fallbackFactory: () => Observable<T>): Observable<T> {
    if (error instanceof HttpErrorResponse && error.status === 0) {
      return fallbackFactory();
    }

    return throwError(() => normalizeHttpError(error, 'No se pudieron cargar las preferencias de compra.'));
  }
}
