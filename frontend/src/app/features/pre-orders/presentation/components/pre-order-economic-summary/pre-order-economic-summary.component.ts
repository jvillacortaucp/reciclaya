      ...
      <aside class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="mt-4 space-y-3 text-base">
          <div class="flex items-center justify-between">
            ...
            <strong class="text-slate-900">{{ currencySymbol(summary.currency) }} {{ summary.unitPrice }} / Ton</strong>
          </div>
          <div class="flex items-center justify-between">
          ...
          <div class="border-t border-slate-200 pt-3 flex items-center justify-between">
            ...
            <strong class="text-slate-900">{{ currencySymbol(summary.currency) }} {{ summary.subtotal.toFixed(2) }}</strong>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-slate-600">Servicio Logístico</span>
            <strong class="text-slate-900">{{ currencySymbol(summary.currency) }} {{ summary.logisticsFee.toFixed(2) }}</strong>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-slate-600">Tarifa Admin</span>
            <strong class="text-emerald-700">
              {{ summary.adminFee === 0 ? 'Gratis' : currencySymbol(summary.currency) + ' ' + summary.adminFee.toFixed(2) }}
            </strong>
          </div>
        </div>
        ...
        <div class="mt-5 border-t border-dashed border-slate-200 pt-4">
          ...
          <p class="mt-1 text-6xl font-bold text-emerald-700">{{ currencySymbol(summary.currency) }} {{ summary.total.toFixed(2) }}</p>
...
export class PreOrderEconomicSummaryComponent {
  @Input() summary: EconomicSummary | null = null;

  protected currencySymbol(currency: EconomicSummary['currency']): string {
